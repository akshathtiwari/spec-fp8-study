...
configuration while acceptance is not: coefficient of variation 29.9\% over
23 boots. The dispersion was present with CUDA graphs enabled and collapsed
under \tw{-{}-enforce-eager} (CV 3.9\%); the responsible mechanism remains
unidentified. Third, acceptance
length $\tau$ has a boot-to-boot \emph{coefficient of variation of 0.62\%},
which we could not find reported; at our sample size this bounds detectable
precision effects at roughly 1.7\%, and smaller effects have been reported
elsewhere as real.
Fourth, the metric commonly called \emph{inter-token latency is, as
computed by the load generators we examined, inter-chunk latency}: a speculative step emits every accepted token in one
...
Identical configurations have CV 29.9\% across 23 boots at constant
acceptance. We localise this to \emph{draft-model} speculation: n-gram
speculation shows CV 2.7\% across twelve boots. We do not identify the
mechanism. \tw{-{}-enforce-eager} removes the
...
0.62\% across 12 boots, bounding detectable effects at roughly 1.7\% at
this $n$. We report FP8's 0.7\% effect as \emph{not detected at this sample
size}, and withdraw an earlier claim of ours that rested on one boot per
arm.
...
scattered throughput while acceptance stays put. Over 23 boots the
coefficient of variation is \textbf{29.9\%} (median 43.7 tok/s, IQR 27.6).

We report CV rather than the max/min range. A range grows with sample size
by construction, so ranges from arms measured with different boot counts
cannot be compared --- an earlier revision of this paper contrasted a
23-boot range against an 8-boot one, and subsampling the 23 boots to $n=8$
shows roughly a fifth of that contrast was sample size rather than
behaviour.
...
default mean). Disabling CUDA graphs collapses the dispersion to CV 3.9\%
over 12 boots.
...
What is stable is the \emph{dispersion}: CV 29.9\% over 23 default boots
against 3.9\% over 12 eager boots, and every eager boot exceeds every
...
across twelve boots --- CV 2.7\% against DFlash's 29.9\% --- and no benefit
from \tw{-{}-enforce-eager}. These are the same
...
Twelve boots cannot prove absence; they bound it. DFlash's 23 boots give
CV 29.9\% and n-gram's twelve give 2.7\%, an order of magnitude apart, so
if n-gram shared DFlash's dispersion, drawing twelve boots that tight would
be extraordinarily unlikely. We report this as a localisation, not an
exclusion. The
...
\paragraph{What the data supports.} At $n = 12$, a crude two-sample
minimum detectable effect at roughly 80\% power is \textbf{1.74\%} of the
mean. The FP8 precision effect we measure is 0.7\%. The defensible statement
is therefore that \emph{a precision effect of 0.7\% is not detected at this
sample size}, and that detecting an effect that small would need
substantially more boots --- not that it is unfalsifiable or absent.
...
accepted draft token at once. So the quantity commonly called inter-token
latency is, as computed by the load generators we examined, \emph{inter-chunk}
latency, and on a speculative server it exceeds the amortised per-token
interval by roughly $\tau$.
...
concurrency, using it to gate a deployment inverts the decision. Prior work
