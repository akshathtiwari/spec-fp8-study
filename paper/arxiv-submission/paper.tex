...
throughput varies \emph{$2.96\times$ across boots} of an identical
configuration while acceptance does not; the variance is attributable to the
CUDA-graph execution path, and disabling it collapses the spread to
$1.04\times$. Third, the acceptance
...
Identical configurations differ by $2.96\times$ across 23 boots at constant
acceptance. We localise this to \emph{draft-model} speculation: n-gram
speculation shows $1.10\times$ across twelve boots. \tw{-{}-enforce-eager} removes the
...
throughput anywhere in a $2.96\times$ band. Acceptance is unchanged
...
Pinning \tw{cudagraph\_mode: PIECEWISE} lands at the low end of the range
(30.04 versus a 30.57 default mean), which is consistent with the fallback
being what the default path degrades to. Disabling CUDA graphs removes the
variance entirely.
...
What is stable is the \emph{spread}, and it is stable in both directions:
23 default boots span $2.96\times$, while 8 eager boots span $1.04\times$
and every one of them exceeds every default boot. Turning CUDA graphs off
does not reliably make speculative decoding faster by a stated factor --- it
makes it \emph{reproducible}, and whatever throughput it gains is however
far down the band the default would otherwise have fallen.
...
protocol with n-gram (prompt-lookup) speculation finds no such variance
across twelve boots --- spread $1.10\times$ against DFlash's $2.96\times$ ---
and no benefit from \tw{-{}-enforce-eager} ($1.00\times$). These are the same
...
Twelve boots cannot prove absence; they bound it. DFlash's 23 boots span
$2.96\times$ and n-gram's twelve span $1.10\times$, so if n-gram shared
DFlash's distribution, drawing twelve boots that tight would be
extraordinarily unlikely. We report this as a localisation, not an
exclusion. The
